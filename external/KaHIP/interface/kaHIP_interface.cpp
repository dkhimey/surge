...
// An input partition is usable as a warm start only if every entry names a
// block in range and no block is left empty -- refinement and the boundary data
// structures assume all k blocks exist.
bool internal_input_partition_is_valid( const int* input_part, int n, int k) {
        if( input_part == NULL || k <= 0 ) return false;

        std::vector<bool> block_seen(k, false);
        int blocks_used = 0;
        for( int node = 0; node < n; node++) {
                int block = input_part[node];
...
void internal_kaffpa_call(PartitionConfig & partition_config, 
        ...
        // Seed the warm start after internal_build_graph: configurate_balance()
        // clears graph_allready_partitioned, so the flag has to be set last.
        if( input_part != NULL ) {
                forall_nodes(G, node) {
                        G.setPartitionIndex(node, input_part[node]);
                } endfor
...
void kaffpa_warmstart(int* n,
        ...
        // Keep the given partition instead of computing a new one, and let
        // initial_partitioning seed its incumbent from it.
        partition_config.no_new_initial_partitioning = true;
        partition_config.omit_given_partitioning     = false;

        // Confine GPA path growth to within blocks. Cross-block pairs are
        // discarded by gpa_matching anyway once the graph is partitioned, so
        // growing through them only thins the matching; same choice KaHIP makes
        // for its own V-cycles in initial_refinement.
        partition_config.gpa_grow_paths_between_blocks = false;

        // perform_partitioning() zeroes every partition index before each
        // repetition, which would discard the warm start.
        partition_config.repetitions = 1;

        // Copy first: part is both the input and the output buffer.
        std::vector<int> input_part(part, part + *n);
        internal_kaffpa_call(partition_config, suppress_output, n, vwgt, xadj, adjcwgt, adjncy, nparts, imbalance, false, edgecut, part, input_part.data());
}
...
void kaffpa_balance(int* n, 
        ...
        partition_config.seed = seed;
        internal_kaffpa_call(partition_config, suppress_output, n, vwgt, xadj, adjcwgt, adjncy, nparts, imbalance, perfectly_balance, edgecut, part, NULL);
}
