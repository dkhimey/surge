...
// same as kaffpa, but warm-started from a given partition.
// On entry part must hold a valid assignment: every entry in [0,nparts) and
// every block non-empty. That assignment is used instead of computing a fresh
// initial partitioning, and is only improved locally, so block labels carry
// over to the result and callers need no relabeling step. Falls back to a cold
// kaffpa run if the input assignment is not valid.
void kaffpa_warmstart(int* n, int* vwgt, int* xadj,
                   int* adjcwgt, int* adjncy, int* nparts,
                   double* imbalance, bool suppress_output, int seed, int mode,
                   int* edgecut, int* part);
